...
export async function setServerConfig(): Promise<void> {
  ...
  const commandQueue = await channel.assertQueue('command', { autoDelete: true });
  ...
  const userExchange = await channel.assertExchange(userExchangeKey, 'fanout');
  await channel.bindExchange(userExchange.exchange, chatExchange.exchange, userExchangeBindPattern);

  console.log('Assert user queue...🚀\n');
  const userQueue = await channel.assertQueue(userExchangeKey, { autoDelete: true });
